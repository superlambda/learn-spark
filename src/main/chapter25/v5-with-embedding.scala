...
//  selectedFields = selectedFields.drop(field)
  val oheEncoder = new OneHotEncoder()
    ...
    // Fit the encoder and then transform the DataFrame
    val oheModel = oheEncoder.fit(selectedFields)
    selectedFields = oheModel.transform(selectedFields)

    // Drop the original indexed field after encoding
    selectedFields = selectedFields.drop(field)
}
